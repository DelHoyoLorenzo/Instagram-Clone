...
<div class="container">
    <div class="row">
        ...
        @foreach($posts as $post)
            <div class="col-6 offset-2 py-3 d-flex gap-3 text-primary align-items-center">
                <div class="w-25">
                    <a class="text-decoration-none" href="/profile/{{$post->user->id}}">
                        <img class="w-100 p-0 m-0 rounded-circle" src="{{ $post->user->profile->profileImage() }}" alt="user-profile-image">
                    </a>
                </div>
                <div class="pl-1" style="width: 100%;">
                    <p class="fw-bold m-0"><a class="text-decoration-none" href="/profile/{{$post->user->id}}">{{ $post->user->username }}</a></p>
                </div>
            </div>
            {{-- image --}}
            ...
            <div class="col-6 offset-2 py-2">
                ...
                <div class="d-flex gap-1 my-2">
                    <div style="cursor: pointer;" class="likeButton" id="LikeButton" class="like-button" liked={{ $post->likes }} post_id={{ $post->id }} user_id={{ auth()->user()->id }} key={{ $post->id }} ></div>
                    <a href="/p/{{ $post->id }}">
                ...
                {{-- @include('components.likesCount', ['true_post_likes' => $true_post_likes]) --}}
                {{-- <x-likesCount :message={{'' ?? $message}} /> --}}
                {{-- <x-like-count :postId="$post->id" @click="fetchLikeData({{ $post->id }})" /> --}}
                ...
                <a class="text-decoration-none m-0 p-0" href="/p/{{ $post->id }}">
                    <p class="p-0 my-2 text-comment" >View all <span>{{ $post->comments->count() || '' }}</span> comments...</p>
                </a>
            </div>
        ...
        <div class="col-3 py-4 d-flex flex-column">
            <div class="col-6 d-flex align-items-center w-100">
                    <div class="w-25">
                        <a class="text-decoration-none" href="/profile/{{auth()->user()->id}}">
                            <img class="w-100 rounded-circle" src="{{ auth()->user()->profile->profileImage() }}" alt="user-profile-image">
                        </a>
                    </div>
                    <div class="d-flex flex-column align-items-baseline" style="margin-left: 10px;">
                        <a class="text-decoration-none" href="/profile/{{auth()->user()->id}}">
                            <p class="fw-bold m-0">{{ auth()->user()->username }}</h3>
