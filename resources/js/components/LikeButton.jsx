...
function LikeButton(props) {
    ...
    const [isLiked, setIsLiked] = useState(()=>{
        const foundLike = liked?.some((like) => like.user_id === +user_id && like.like);
        return foundLike
    });

    const [trueLikesCount, setTrueLikesCount] = useState(()=>{
        let sum = 0;
        liked?.forEach(like => {
            if(like.like){
                sum++
    ...
    const likePost = () =>
    {
        axios.post(`http://localhost:8000/like/${post_id}`,{
            'like': true,
            'post_id': post_id,
        })
        .then(response => {
            setIsLiked(!isLiked)
            setTrueLikesCount(response.data.true_post_likes)
        })
        .catch(error => {
            console.log(error)
        });
  ...
  return (
    <div onClick={ likePost } className='cursor-pointer'>
        ...
        <p className='fw-bold pt-2 m-0 text-white'>{trueLikesCount} likes</p>
    </div>
